...
| `aweek report` | `skills/report/SKILL.md` | Agent → CEO report. Sender agent writes a notification (`kind: 'report' \| 'question'`) under its own `.aweek/agents/<slug>/notifications.json` AND, when the project's Slack config carries a `ceoChannel`, pushes a Block Kit message to that channel/DM via `chat.postMessage`. Used autonomously by agents during a heartbeat tick (`aweek exec report send …` over Bash) and exposed as a slash command for manual composition / testing. Slack delivery is per-call (subscribed inside the same short-lived `aweek exec` process that persists the notification), so it does NOT depend on `aweek serve` being up |
| `aweek config` | `skills/config/SKILL.md` | CLI counterpart to the dashboard Settings page. Renders every knob `.aweek/config.json` exposes (`timeZone`, `staleTaskWindowMs`, `heartbeatIntervalSec`, `runner`) and edits any of them via an interactive picker that doubles as the confirmation gate. Editing `heartbeatIntervalSec` additionally calls `installHeartbeat` to rotate the live launchd plist's `StartInterval` in the same flow |
...
4. Launches a coding-agent CLI session (`src/execution/cli-session.ts`) with the task prompt + the subagent identity. Which CLI runs is the **execution runner** (see below). The session executor (`src/execution/session-executor.ts`) records token usage automatically.
...
### Execution runners (Claude / Gemini / Hermes)

The CLI that actually runs a task is pluggable via `src/execution/runner.ts` (`RunnerKind = 'claude' | 'gemini' | 'hermes'`). Resolution precedence is **per-agent `runner` field** (`.aweek/agents/<slug>.json`) → **project-wide `.aweek/config.json` `runner`** → **`DEFAULT_RUNNER` (`'claude'`)** — implemented by `resolveRunner(agent.runner, config.runner)`. The heartbeat resolves the runner once per tick in `runHeartbeatForAgent` and threads it through `ExecutionContext` → `executeSessionWithTracking`; `aweek run-once` resolves it the same way. Set it with `aweek config` (the `runner` knob) or per-agent by editing the agent JSON.

`cli-session.ts` owns the per-runner differences; `runner.ts` owns the vocabulary (kinds, default, `RUNNER_BINARY` map, the `GEMINI_SYSTEM_MD` env-var name) so config, the agent schema, the heartbeat, and the CLI all agree on one set of strings.

- **`claude`** — `claude --print --output-format stream-json --verbose --agent <slug> --append-system-prompt <runtimeContext> <taskPrompt>` (`buildCliArgs`). Identity is resolved natively by `--agent`; scheduling context is layered with `--append-system-prompt`. Token usage is read from the terminal `{type:"result"}` event's `usage` object by `parseTokenUsage`.
- **`gemini`** — `gemini --output-format stream-json --prompt <runtimeContext + taskPrompt> [--yolo --skip-trust] [--model M]` (`buildGeminiCliArgs`). The Gemini CLI has no `--agent` / `--append-system-prompt`, so: (a) the subagent's `.claude/agents/<slug>.md` is injected as the system prompt via the `GEMINI_SYSTEM_MD` env var (set in `launchSession`, resolved across project + user scope by `resolveSubagentFile`; skipped silently when absent so the `.md` stays the single source of truth for identity); (b) the per-tick runtime context is prepended to the task prompt. `--yolo` (auto-approve all tools) + `--skip-trust` (trust the workspace, no first-run dialog) are the headless equivalent of Claude's `--dangerously-skip-permissions` and are emitted only when `dangerouslySkipPermissions` is set. A Claude model alias is never forwarded to Gemini — `--model` is added only when a Gemini model is explicitly requested. Token usage lives in the terminal `{type:"result"}` event's **`stats`** object (`stats.input_tokens` / `output_tokens` / `total_tokens`, NOT `usage`) and is read by `parseGeminiTokenUsage`.
- **`hermes`** — `hermes --oneshot <identity + runtimeContext + taskPrompt> [--yolo --accept-hooks] [--model M]` (`buildHermesCliArgs`). The Hermes Agent CLI has no `--agent`, no system-prompt flag, and no system-prompt env var, so BOTH the subagent identity (the `.md` **body**, read+stripped via `parseSubagentBody` in `launchSession`, resolved across project + user scope) and the runtime context are embedded directly at the head of the one-shot prompt. `--yolo` (bypass dangerous-command approvals) + `--accept-hooks` (auto-approve unseen shell hooks without a TTY) are the headless no-permission pair, emitted only when `dangerouslySkipPermissions` is set; one-shot mode already auto-bypasses approvals, but `--yolo` is passed explicitly so the contract doesn't depend on one-shot defaults. Hermes one-shot prints only final response text — **no usage/cost metadata** — so `parseTokenUsageForRunner('hermes', …)` returns `null` and usage tracking degrades to "untracked" (the session still runs and is logged).
...
**Library dep.** Slack adapter and streaming bridge come from the `agentchannels` npm package (`^0.2.0`), wired in via the standard `dependencies` block in `package.json`. The public exports `SlackAdapter`, `StreamingBridge`, `SlackManifestAPI`, and the `Backend` / `ResolveBackendHook` / `BackendRegistry` types live on the package's main entry point. Bumping the dep range is a normal package upgrade — no `file:` workspace dance is needed.
...
### Agent → CEO report (`aweek report` / outbound Slack push)

Agents emit ad-hoc reports/questions to the user (CEO) via the `aweek report` skill (`skills/report/SKILL.md`, dispatcher entry `report.send` → `src/skills/report.ts`'s `reportToCeo`). This is **independent of the inbound Slack listener** above — the listener is `aweek serve`-resident and drains DMs/mentions over Socket Mode; the report path lives entirely inside the SHORT-LIVED `aweek exec report send` process the agent spawns during a heartbeat tick.

**Per-call fan-out.** `reportToCeo` persists a notification through `NotificationStore.send()` under `.aweek/agents/<sender>/notifications.json` with auto-stamped `metadata = { kind: 'report' | 'question', channel: 'ceo-report' }` (caller-supplied metadata keys win on conflict). Before the persist call, it loads Slack credentials via `loadSlackCredentials` (env-first, then `.aweek/channels/slack/config.json`) and — when BOTH `botToken` and the optional `ceoChannel` field are present — instantiates a `SlackNotificationDelivery` channel (`src/channels/slack/notification-delivery.ts`) and `store.subscribe()`s it (wrapped in a tracking layer that captures the per-call delivery outcome). After `send()` resolves, the skill calls `await notificationStore.drain()` which awaits every in-flight async `deliver()` promise the fanout registered. The drain is load-bearing: short-lived CLI invocations (`aweek exec report send`) follow `process.exit(0)` in `bin/aweek.ts`, which would otherwise abort the in-flight `fetch()` to slack.com — the user sees `deliveredToSlack: true` for a message that never landed. The drain guarantees the POST has completed (success or Slack `ok: false` error) before the skill returns; `deliveredToSlack` then reflects the **actual outcome** (`true` iff Slack returned `ok: true`; `false` for no-channel-configured, Slack-side failures, or fetch errors — each visible in stderr via the `onSlackError` sink that defaults to a single warning line so a misconfigured `ceoChannel` doesn't silently swallow failures).

**Why per-call, not server-resident.** `NotificationStore.subscribe()` is process-local. The heartbeat spawns `claude --print` which spawns `aweek exec report send` as a grandchild process — *that* grandchild is the one writing the notification, and the long-running `aweek serve` never sees the write. A server-side subscriber would never fire. So the report path attaches the channel inline in the same process that calls `send()`. The trade-off: the `aweek serve` Slack adapter is INBOUND-only and the report path is OUTBOUND-only — they share credential loading (`slack-config-store`) but nothing else. There's no `--dangerously-skip-permissions` involvement (that flag is Slack-bridge-only, not report-skill).

**Block Kit formatting.** `buildSlackMessage` in `src/channels/slack/notification-delivery.ts` renders one Slack message per notification — a `header` block carrying a severity emoji (`:information_source:` / `:warning:` / `:rotating_light:` for `info` / `warning` / `error`, matching the canonical `NotificationSeverity` enum in `src/schemas/notification.schema.js`) plus the title, a `section` block with the body (truncated with a tail `…` at the 2900-char per-block Slack limit), and a `context` block listing the kind label, sender slug, optional `link`, and optional `sourceTaskId` traceback. The `text` fallback (used for OS push notifications and clients that don't render Block Kit) mirrors the title + sender.

**Credential surface.** `SlackCredentials` in `src/storage/slack-config-store.ts` carries an OPTIONAL `ceoChannel` field alongside the existing `botToken` / `appToken` / `signingSecret`. The loader accepts the channel ID via env (`SLACK_CEO_CHANNEL`, takes precedence) or via any of the camelCase (`ceoChannel`), snake_case (`ceo_channel`), or screaming-snake (`SLACK_CEO_CHANNEL`) keys in `.aweek/channels/slack/config.json`. When absent, `deliveredToSlack: false` is returned and only the dashboard inbox gets the write — there's no error. The Slack target accepts channel IDs (`C…`/`G…`), user IDs (`U…`), and DM IDs (`D…`); `chat.postMessage` handles all three.

**Slack thread → project Claude context bridge.** When a CEO replies to a report in Slack, the reply has to land back as a project-level Claude turn with the original report's content already on the wire — otherwise the user gets a cold-start chat that doesn't know what they're replying to. The bridge is split across three pieces:

1. **`src/storage/slack-report-thread-store.ts`** — a sibling of `slack-thread-store.ts` that maps `slack:<channel>:<ts>` to `{ senderSlug, kind, title, body, sourceTaskId?, postedAt }`. Lives at `.aweek/channels/slack/report-threads/<encodedKey>.json`. Same filename encoder (`encodeThreadKey`) as `slack-thread-store` so the two stores share a slug shape; same atomic-write contract (`writeFile.tmp` + `rename`); lazy GC on read. **30-day TTL**, intentionally longer than `slack-thread-store`'s 24h DM TTL so a CEO who replies days later still gets the report context injected (the chat-thread session id would have GC'd by then; the report record survives and seeds a fresh session).
2. **`src/channels/slack/notification-delivery.ts:SlackNotificationDelivery`** parses Slack's `chat.postMessage` success response (`{ ok: true, channel, ts }`) and invokes an optional `onPosted({ channel, ts }, notification)` callback BEFORE `deliver()` resolves. `src/skills/report.ts` wires that callback to `saveReportThread()` with `threadKey: \`slack:${channel}:${ts}\`` — note Slack's response `channel` is canonical (a user-ID target like `U…` returns the corresponding DM channel `D…`, which is what the inbound `SlackAdapter` will emit on the eventual reply, so we always trust the response value).
3. **`src/serve/slack-bridge.ts` + `src/channels/slack/backend-factory.ts`** — the inbound bridge's `resolveBackend` hook passes a `loadFirstTurnSystemPromptAppend` callback to the factory. The factory ONLY invokes that callback when there is no existing `slack-thread-store` record (i.e., the chat-session-cold-start branch — either a brand-new Slack thread or one whose 24h DM TTL has GC'd the session id). The callback in turn calls `loadReportThread(dataDir, ctx.threadKey)` and, if a record is found, returns the formatted banner from `buildReportContextBanner(record)`. The factory appends the banner to the conversational banner (joined by a blank line) and passes the combined string as `--append-system-prompt` to the spawn. **Why first-turn-only:** Claude Code CLI's `--resume` reloads the session with its frozen system prompt; re-injecting the report context on resumed turns is at best wasted tokens, at worst confusing duplicate context. The factory's `existing === null` gate keeps the injection one-shot.

After the first reply has been answered, the bridge's normal flow takes over: the freshly minted `claudeSessionId` lands in `slack-thread-store` via the existing `onSessionInit` hook, and every subsequent reply in the same Slack thread follows the `--resume <sessionId>` path — same as any other Slack DM chat. The report-thread record stays on disk for 30 days but is silently ignored on resumed turns. v1 routes to **project-level Claude** (no `--agents <slug>` flag); the banner makes the agent slug visible so project Claude can dispatch back to the originating subagent via `Task()` or `aweek exec` if the user asks for changes to the agent's plan, but explicitly tells Claude NOT to impersonate the agent.

**Isolation contract.** The report path MUST NOT acquire a per-agent heartbeat lock (`src/lock/lock-manager.ts`) — reports are agent-emitted side-effects within an already-locked heartbeat tick, so re-locking would deadlock. It also MUST NOT touch the weekly-budget pause flag — a CEO report consumes tokens via its parent Claude session, not as a separate budget event. The colocated test suite (`src/skills/report.test.ts` + `src/channels/slack/notification-delivery.test.ts` + `src/storage/slack-report-thread-store.test.ts` + the new `loadFirstTurnSystemPromptAppend` cases in `src/channels/slack/backend-factory.test.ts`) pins all three invariants by asserting only the per-sender `notifications.json` mutates plus, when wired, exactly one `chat.postMessage` POST per call AND the report-thread record lands at `slack:<channel>:<ts>` AND the first-turn callback is invoked iff `existing === null`.
