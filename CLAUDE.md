...
**Per-call fan-out.** `reportToCeo` persists a notification through `NotificationStore.send()` under `.aweek/agents/<sender>/notifications.json` with auto-stamped `metadata = { kind: 'report' | 'question', channel: 'ceo-report' }` (caller-supplied metadata keys win on conflict). Before the persist call, it loads Slack credentials via `loadSlackCredentials` (env-first, then `.aweek/channels/slack/config.json`) and — when BOTH `botToken` and the new optional `ceoChannel` field are present — instantiates a `SlackNotificationDelivery` channel (`src/channels/slack/notification-delivery.ts`) and `store.subscribe()`s it. The store's existing `_fanout()` then fires `deliver()` post-write. `NotificationDeliveryChannel.deliver()` is fire-and-forget from the store's perspective; the Slack POST runs as an unawaited `fetch` against `https://slack.com/api/chat.postMessage` and any `{ok: false, error}` response surfaces through the per-call `onChannelError` sink (defaults to a stderr warning line so a misconfigured `ceoChannel` doesn't silently swallow failures).
