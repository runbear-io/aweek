...
function makeScheduledTask(partial) {
...
describe('CalendarGrid — task placement', () => {
  ...
  it('numbers tasks in column-major order', () => {
    ...
    const byId = (id) => container.querySelector(`[data-task-id="${id}"]`);
...
describe('CalendarGrid — task cell rendering parity (Sub-AC 3)', () => {
  ...
  it('extractMinuteBadge handles slot.minute, runAt fallback, and zero-minute tasks', () => {
    ...
    expect(
      extractMinuteBadge({ slot: { minute: 45 }, runAt: null }),
    ).toBe('45');
    ...
    expect(
      extractMinuteBadge({ slot: { minute: 0 }, runAt: '2026-04-20T09:00:00.000Z' }),
    ).toBeNull();
