 ...
 */
function makeUsage(profile: typeof ALICE, overrides = {}) {
  const defaults = {
    slug: profile.slug,
    name: profile.name,
 ...
 */
async function renderReady(profile: typeof ALICE, stubOpts?: Parameters<typeof makeFetchStub>[1]) {
  const fetch = makeFetchStub(profile, stubOpts);
  const utils = render(<AgentProfilePage slug={profile.slug} fetch={fetch as unknown as typeof globalThis.fetch} />);
  await waitFor(() => {
...
describe('AgentProfilePage — budget / usage summary', () => {
  ...
  it('flags over-budget state with the "over budget" pill', async () => {
    ...
    await renderReady(over as unknown as Parameters<typeof renderReady>[0]);
  ...
  it('renders the usage-endpoint breakdown (input / output / cost / records)', async () => {
    ...
    const { container } = render(<AgentProfilePage slug={ALICE.slug} fetch={fetch as unknown as typeof globalThis.fetch} />);
    await waitFor(() => {
    ...
    // (we key off data-field rather than getByTestId)
...
describe('AgentProfilePage — error + empty states', () => {
  it('shows a not-found empty state when the API returns 404', async () => {
    ...
    const { container } = render(
      <AgentProfilePage slug="ghost" fetch={fetch as unknown as typeof globalThis.fetch} />,
    );
    await waitFor(() => {
