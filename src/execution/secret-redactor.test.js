...
describe('secret-redactor — redactLine', () => {
  ...
  it('redacts Anthropic `sk-ant-…` keys', () => {
    const out = redactLine('token=sk-ant-api03-aB3_XyZ-1234567890abcdefghij');
  ...
  it('redacts GitHub fine-grained PATs', () => {
    ...
    const out = redactLine(`token=${token}`);
    assert.ok(out.includes(M));
  ...
  it('redacts AWS access-key IDs', () => {
    const out = redactLine('aws_key=AKIAIOSFODNN7EXAMPLE uploaded');
    assert.ok(out.includes(M));
  ...
  it('redacts Google API keys', () => {
    ...
    const out = redactLine(`key=${google} ok`);
    assert.ok(out.includes(M));
    assert.ok(!out.includes(google));
  });

  it('redacts Stripe keys', () => {
    const out = redactLine('use sk_' + 'live_51abcdefghijklmnopqrstuvwxyz for billing');
    assert.ok(out.includes(M));
  ...
  it('redacts JWTs', () => {
    ...
    const out = redactLine(`Authorization: ${jwt}`);
    assert.ok(out.includes(M));
    assert.ok(!out.includes(jwt));
  });

  it('redacts Bearer tokens', () => {
    const out = redactLine('Authorization: Bearer abcDEF0123456789XYZ_token-value');
    assert.ok(out.includes(M));
    assert.ok(!out.includes('abcDEF0123456789XYZ_token-value'));
  });

  it('redacts NAME=value style env-var secrets, keeping the name', () => {
    const out = redactLine('OPENAI_API_KEY=sk-xyz789abcdef012345');
  ...
  it('redacts NAME: value style secrets too', () => {
    const out = redactLine('MY_SECRET: hunter2isNotEnough');
  ...
  it('preserves surrounding JSON punctuation', () => {
    const out = redactLine('{"apiKey":"sk-abcDEF0123456789xyz","other":"value"}');
