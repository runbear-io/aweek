...
describe('weekly task schema — runAt', () => {
  const makeTask = () => {
    const goal = createGoal('Test goal', '3mo');
    const obj = createObjective('Test objective', goal.id);
  ...
  it('accepts a task with a runAt ISO date-time', () => {
    const task = makeTask();
  ...
  it('accepts a task without runAt (optional field)', () => {
    const plan = createWeeklyPlan('2026-W17', '2026-04', [makeTask()]);
    assert.equal(validateWeeklyPlan(plan).valid, true);
  });

  it('rejects a task whose runAt is not a valid date-time', () => {
    const task = makeTask();
    task.runAt = 'not-a-date';
    const plan = createWeeklyPlan('2026-W17', '2026-04', [task]);
    const result = validateWeeklyPlan(plan);
    assert.equal(result.valid, false);
    assert.ok(
      result.errors.some((e) => /runAt/.test(JSON.stringify(e))),
      'expected a runAt-related error',
    );
  });

  it('createTask writes runAt into the task when passed via opts', () => {
    const goal = createGoal('Test goal', '3mo');
    const obj = createObjective('Test objective', goal.id);
    const task = createTask('Publish post', obj.id, {
