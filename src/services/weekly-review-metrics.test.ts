...
type MetricsParams = Parameters<typeof formatMetricsSection>[0];
type AggregateDeps = Parameters<typeof aggregateWeeklyMetrics>[0];
...
describe('formatMetricsSection', () => {
  const baseMetrics: MetricsParams = {
    tasks: {
      completed: 5,
      failed: 1,
      skipped: 0,
...
describe('aggregateWeeklyMetrics', () => {
  ...
  function deps(): AggregateDeps {
    return {
      activityLogStore: activityLogStore as unknown as AggregateDeps['activityLogStore'],
      usageStore: usageStore as unknown as AggregateDeps['usageStore'],
      inboxStore: inboxStore as unknown as AggregateDeps['inboxStore'],
    };
  }
  ...
  it('aggregates token usage from usage store', async () => {
    const record1 = createUsageRecord({
      ...
      costUsd: 0.02,
      week: weekMonday,
    } as Parameters<typeof createUsageRecord>[0]);
  ...
  it('handles store errors gracefully', async () => {
    ...
    const badDeps: AggregateDeps = {
      activityLogStore: activityLogStore as unknown as AggregateDeps['activityLogStore'],
      usageStore: badUsageStore as unknown as AggregateDeps['usageStore'],
      inboxStore: badInboxStore as unknown as AggregateDeps['inboxStore'],
    };
    const { metrics } = await aggregateWeeklyMetrics(
      badDeps,
      agentId,
