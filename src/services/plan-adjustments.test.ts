...
interface AgentConfigShape {
  ...
  subagentRef: string;
  goals: GoalShape[];
  monthlyPlans: { month: string; objectives: ObjectiveShape[] }[];
...
interface TestAgentBundle {
  config: AgentConfigShape;
  weeklyPlans: AgentWeeklyPlanShape[];
  weeklyPlan: AgentWeeklyPlanShape;
  goal1: GoalShape;
  goal2: GoalShape;
  obj1: ObjectiveShape;
  obj2: ObjectiveShape;
...
function buildTestAgent({ subagentRef = TEST_SLUG }: { subagentRef?: string } = {}): TestAgentBundle {
  const config = createAgentConfig({
    subagentRef,
    weeklyTokenLimit: 100_000,
  }) as AgentConfigShape;

  const goal1 = createGoal('Improve code quality', '3mo') as GoalShape;
  const goal2 = createGoal('Write documentation', '1mo') as GoalShape;
  addGoal(config, goal1);
  addGoal(config, goal2);

  const obj1 = createObjective('Refactor module A', goal1.id) as ObjectiveShape;
  const obj2 = createObjective('Write API docs', goal2.id) as ObjectiveShape;
  const monthlyPlan = createMonthlyPlan('2026-04', [obj1, obj2]) as { month: string; objectives: ObjectiveShape[] };
  config.monthlyPlans.push(monthlyPlan);

  const task1 = createTask({ title: 'Refactor utils.js', prompt: 'Refactor utils.js' }, obj1.id) as TaskShape;
  const task2 = createTask({ title: 'Draft API overview', prompt: 'Draft API overview' }, obj2.id) as TaskShape;
  const weeklyPlan = createWeeklyPlan('2026-W16', '2026-04', [task1, task2]) as AgentWeeklyPlanShape;
  const weeklyPlans: AgentWeeklyPlanShape[] = [weeklyPlan];
...
describe('applyGoalAdjustment', () => {
  it('adds a goal', () => {
    ...
    assert.equal((r.result as GoalShape).description, 'Ship v2');
    assert.equal((r.result as GoalShape).horizon, '1yr');
  });
  ...
  it('updates a goal description and horizon', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal((r.result as GoalShape).description, 'Updated desc');
    assert.equal((r.result as GoalShape).horizon, '1yr');
  });
  ...
  it('updates a goal status to completed', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal((r.result as GoalShape).status, 'completed');
    assert.ok((r.result as GoalShape).completedAt);
  });
  ...
  it('removes a goal', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal(config.goals.length, before - 1);
    assert.equal((r.result as { removed?: boolean }).removed, true);
  });
...
describe('applyMonthlyAdjustment', () => {
  it('adds an objective to a monthly plan', () => {
    ...
    assert.ok((r.result as ObjectiveShape).id.startsWith('obj-'));
  });
  ...
  it('updates an objective status', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal((r.result as ObjectiveShape).status, 'completed');
  });
  ...
  it('updates an objective description', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal((r.result as ObjectiveShape).description, 'Revised objective');
  });
...
describe('applyWeeklyAdjustment', () => {
  it('adds a task to a weekly plan', () => {
    ...
    assert.ok((r.result as TaskShape).id.startsWith('task-'));
  });
  ...
  it('updates a task status to completed', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal((r.result as TaskShape).status, 'completed');
    assert.ok((r.result as TaskShape).completedAt);
  });
  ...
  it('updates a task title and prompt', () => {
    ...
    assert.equal(r.applied, true);
    assert.equal((r.result as TaskShape).title, 'Revised task');
    assert.equal((r.result as TaskShape).prompt, 'Revised task');
  });
...
describe('adjustGoals', () => {
  ...
  it('adds a goal and persists', async () => {
    ...
    // Verify persistence
    const reloaded = await store.load(config.id) as AgentConfigShape;
  ...
  it('removes a goal and persists', async () => {
    ...
    assert.ok(!reloaded.goals.find((g: GoalShape) => g.id === goal1.id));
  });
  ...
  it('adds a weekly task and persists', async () => {
    ...
    const weeklyPlanStore = new WeeklyPlanStore(tmpDir);
    const reloadedPlan = await weeklyPlanStore.load(config.id, '2026-W16') as AgentWeeklyPlanShape;
  ...
  it('is idempotent for status updates', async () => {
    ...
    const reloaded = await store.load(config.id) as AgentConfigShape;
    const goal = reloaded.goals.find((g: GoalShape) => g.id === goal1.id);
...
describe('adjustGoals subagent-wrapper invariants', () => {
  let tmpDir: string;       // serves as both projectDir and the parent of dataDir
  let dataDir: string;      // .aweek/agents
  let mdPath: string;       // .claude/agents/<slug>.md
  let mdContents: string;   // canonical bytes pinned at setup
  let mdMtimeMs: number;    // mtime in ms pinned at setup
  let store: AgentStore;
  ...
  it('leaves the subagent .md byte-for-byte unchanged after a monthly adjustment', async () => {
    ...
    const before = await store.load(TEST_SLUG) as AgentConfigShape;
  ...
  it('mutates only scheduling fields on the persisted aweek JSON', async () => {
    ...
    const after = await store.load(TEST_SLUG) as AgentConfigShape;
    ...
    assert.equal(
      after.monthlyPlans[0]!.objectives.find((o: ObjectiveShape) => o.id === objectiveId)!.status,
      'in-progress',
      'objective status updated',
    );
    // Weekly plans live in the file store — load and count there.
    const weeklyPlanStore = new WeeklyPlanStore(dataDir);
    const planBefore = await weeklyPlanStore.load(TEST_SLUG, '2026-W16') as AgentWeeklyPlanShape;
