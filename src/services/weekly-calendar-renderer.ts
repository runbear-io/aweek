...
export function statusIcon(status: string | undefined | null): string {
  if (status == null) return STATUS_ICONS.unknown;
...
export function renderDayCell(
  ...
  const tasks: CalendarTask[] = planned.tasks || [];
  ...
  const actualEntries = (actual.entries || []) as CalendarLogEntry[];
 ...
 */
export function renderWeeklySummary(weeklyData: WeeklyData): string[] {
 ...
 */
export function renderCompactCalendar(weeklyData: WeeklyData): string[] {
  const lines: string[] = [];
