...
export interface DelegationMetrics {
  received: DelegationCounts;
  sent: DelegationCounts;
}
...
interface ActivityLogStoreLike {
  summary(agentId: string, weekMonday: string): Promise<ActivitySummary>;
}

interface UsageStoreLike {
  weeklyTotal(agentId: string, weekMonday: string): Promise<UsageTotalInput>;
}

interface InboxStoreLike {
  load(agentId: string): Promise<InboxMessage[]>;
}
...
export function computeTaskMetrics(
  ...
  weeklyPlan: WeeklyPlanInput | null = null,
): TaskMetrics {
 ...
 */
export function computeTokenMetrics(usageTotal: UsageTotalInput | null | undefined): TokenMetrics {
  if (!usageTotal) {
    return {
      inputTokens: 0,
      outputTokens: 0,
...
export function computeDelegationMetrics(
  receivedMessages: InboxMessage[] = [],
  sentMessages: InboxMessage[] = [],
): DelegationMetrics {
  const received: DelegationCounts = {
  ...
  for (const msg of receivedMessages) {
    const status = msg.status as keyof DelegationCounts | undefined;
    if (status && status !== 'total' && received[status] !== undefined) {
      received[status]++;
    }
  }

  const sent: DelegationCounts = {
  ...
  for (const msg of sentMessages) {
    const status = msg.status as keyof DelegationCounts | undefined;
    if (status && status !== 'total' && sent[status] !== undefined) {
      sent[status]++;
    }
  }
 ...
 */
export function formatMetricsSection(metrics: { tasks: TaskMetrics; tokens: TokenMetrics; delegation: DelegationMetrics }): string {
...
export async function aggregateWeeklyMetrics(
  { activityLogStore, usageStore, inboxStore, weeklyPlanStore, agentStore }: AggregateDeps,
  agentId: string,
  weekMonday: string,
  opts: AggregateOpts = {},
): Promise<{ metrics: WeeklyMetrics; markdown: string }> {
  // 1. Task metrics — from activity log + optional plan
  const activitySummary = await activityLogStore.summary(agentId, weekMonday);

  let weeklyPlan: WeeklyPlanInput | null = null;
  ...
  let usageTotal: UsageTotalInput | null = null;
  try {
  ...
  let receivedMessages: InboxMessage[] = [];
  try {
