...
interface TestAgentConfig {
  ...
  subagentRef: string;
  goals: unknown[];
  budget: { weeklyTokenLimit: number; currentUsage: number; periodStart: string };
  createdAt: string;
  updatedAt: string;
  [key: string]: unknown;
}

type GenerateReviewDeps = Parameters<typeof generateWeeklyReview>[0];
...
function makeAgentConfig(overrides: Partial<TestAgentConfig> = {}): TestAgentConfig {
  return {
    id: AGENT_ID,
...
describe('loadReview', () => {
  ...
  it('loads persisted review', async () => {
    ...
    assert.ok(result);
    const meta = result!.metadata as { week: string };
...
describe('generateWeeklyReview', () => {
  ...
  function deps(): GenerateReviewDeps {
    return {
      agentStore: agentStore as unknown as GenerateReviewDeps['agentStore'],
      weeklyPlanStore,
      activityLogStore,
      usageStore,
      inboxStore,
