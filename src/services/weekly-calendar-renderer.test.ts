...
import type { DayComparison, WeeklyData } from './weekly-data-aggregator.js';

// ─── Helpers ────────────────────────────────────────────────────────

interface DayComparisonOverrides {
  date?: string;
  day?: string;
  planned?: DayComparison['planned'];
  actual?: DayComparison['actual'];
}

interface WeeklyDataOverrides {
  agentId?: string;
  week?: string;
  weekMonday?: string;
  planExists?: boolean;
  ...
  days?: DayComparison[];
  unscheduledTasks?: WeeklyData['unscheduledTasks'];
  summary?: WeeklyData['summary'];
}

function makeDayComparison(overrides: DayComparisonOverrides = {}): DayComparison {
  return {
    date: '2026-04-13',
    day: 'mon',
...
function makeWeeklyData(overrides: WeeklyDataOverrides = {}): WeeklyData {
...
describe('renderDayCell', () => {
  ...
  it('shows actual entries when no planned tasks exist', () => {
    const day = makeDayComparison({
      actual: {
        entries: [
          { timestamp: '2026-04-13T10:00:00.000Z', status: 'completed', title: 'Auto task done', taskId: 'inbox-1' },
        ],
        count: 1,
...
describe('renderWeeklyCalendar', () => {
  ...
  it('renders a rich week with mixed statuses', () => {
    ...
    data.days[0] = makeDayComparison({
      ...
      actual: {
        entries: [
          { timestamp: '2026-04-13T10:00:00.000Z', status: 'completed', title: 'Setup CI' },
          { timestamp: '2026-04-13T11:00:00.000Z', status: 'completed', title: 'Write docs' },
        ],
        count: 2,
        statusCounts: { completed: 2 },
        completedCount: 2,
    ...
    data.days[1] = makeDayComparison({
      ...
      actual: {
        entries: [
          { timestamp: '2026-04-14T10:00:00.000Z', status: 'failed', title: 'Deploy staging' },
        ],
        count: 1,
        statusCounts: { failed: 1 },
        completedCount: 0,
