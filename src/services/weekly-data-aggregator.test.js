...
describe('weekDates', () => {
  ...
  it('dates are consecutive', () => {
    ...
    for (let i = 1; i < dates.length; i++) {
      ...
      assert.equal(curr - prev, 86400000, `Gap between ${dates[i - 1].date} and ${dates[i].date}`);
    }
  });
});
...
describe('aggregateWeeklyData', () => {
  let mockWeeklyPlanStore;
  let mockActivityLogStore;
  ...
  it('handles missing weekly plan gracefully', async () => {
    ...
    const result = await aggregateWeeklyData(
      { weeklyPlanStore: noplanStore, activityLogStore: mockActivityLogStore },
      'agent-test',
      '2026-W16'
    );
  ...
  it('handles empty activity log gracefully', async () => {
    const emptyLogStore = { load: async () => [] };
  ...
  it('handles both stores empty', async () => {
    ...
    const result = await aggregateWeeklyData(
      { weeklyPlanStore: noplanStore, activityLogStore: emptyLogStore },
      'agent-test',
      '2026-W16'
    );
