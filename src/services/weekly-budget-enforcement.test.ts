...
interface AgentConfigShape {
  id: string;
  budget?: { weeklyTokenLimit?: number; currentUsage?: number; paused?: boolean };
  [key: string]: unknown;
}

type EnforceBudgetDeps = Parameters<typeof enforceBudget>[1];

describe('weekly-budget-enforcement (integration)', () => {
  let tmpDir: string;
  let agentStore: AgentStore;
  let usageStore: UsageStore;
  let deps: EnforceBudgetDeps;
  ...
  beforeEach(async () => {
    ...
    deps = { agentStore, usageStore, baseDir: tmpDir } as unknown as EnforceBudgetDeps;
  });
  ...
  async function createTestAgent(name: string, weeklyTokenLimit: number = 1000): Promise<AgentConfigShape> {
    const config = createAgentConfig({ subagentRef: name, weeklyTokenLimit } as Parameters<typeof createAgentConfig>[0]) as unknown as AgentConfigShape;
  ...
  describe('pause-on-exhaustion behavior', () => {
    it('persists paused state in agent config', async () => {
      ...
      const config = (await agentStore.load(agent.id)) as AgentConfigShape;
  ...
  describe('alert flag writing', () => {
    ...
    it('does not write duplicate alert on repeated enforcement', async () => {
      ...
      assert.ok(alert);
      assert.equal((alert as { timestamp: string }).timestamp, '2026-04-15T10:00:00Z');
    });
    ...
    it('each week gets its own independent alert file', async () => {
      ...
      const alert1 = (await loadAlert(tmpDir, agent.id, '2026-04-06')) as { weekMonday: string } | null;
      const alert2 = (await loadAlert(tmpDir, agent.id, '2026-04-13')) as { weekMonday: string } | null;
      assert.ok(alert1);
      assert.ok(alert2);
  ...
  describe('multi-agent isolation', () => {
    ...
    it('each agent has independent alert files', async () => {
      ...
      const alert1 = (await loadAlert(tmpDir, a1.id, week)) as { agentId: string; used: number } | null;
      const alert2 = (await loadAlert(tmpDir, a2.id, week)) as { agentId: string; used: number } | null;
  ...
  describe('bound enforcer end-to-end', () => {
    it('full lifecycle through createBudgetEnforcer factory', async () => {
      ...
      const alert = (await enforcer.loadAlert(agent.id, week)) as { type: string; used: number; budget: number } | null;
      assert.ok(alert);
      assert.equal(alert!.type, 'budget-exhausted');
  ...
  describe('idempotency under repeated heartbeats', () => {
    it('repeated enforcement calls produce identical results', async () => {
      ...
      const alert = (await loadAlert(tmpDir, agent.id, week)) as { timestamp: string } | null;
      assert.ok(alert);
  ...
  describe('edge cases', () => {
    ...
    it('large usage far exceeding budget is handled correctly', async () => {
      ...
      const alert = (await loadAlert(tmpDir, agent.id, '2026-04-13')) as { exceededBy: number } | null;
      assert.ok(alert);
