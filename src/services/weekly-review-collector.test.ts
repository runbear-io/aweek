...
interface AgentBudgetConfig {
  ...
  budget?: { weeklyTokenLimit?: number; paused?: boolean };
}
...
function makeTask(overrides: Partial<CollectorTask> = {}): CollectorTask {
  return {
    id: 'task-001',
...
function makeLogEntry(overrides: Partial<CollectorLogEntry> = {}): CollectorLogEntry {
  return {
    id: 'log-aabb0001',
...
type CollectorDeps = Parameters<typeof collectWeeklyReviewData>[0];
type WeeklyPlanStoreLike = CollectorDeps['weeklyPlanStore'];
type ActivityLogStoreLike = CollectorDeps['activityLogStore'];
type UsageStoreLike = NonNullable<CollectorDeps['usageStore']>;
type AgentStoreLike = NonNullable<CollectorDeps['agentStore']>;

// Minimal stub plan store
function makePlanStore(plan: PlanShape | null = null): WeeklyPlanStoreLike {
  return {
    load: async (_agentId: string, _week: string) => {
      if (plan) return plan as Awaited<ReturnType<WeeklyPlanStoreLike['load']>>;
...
function makeLogStore(entries: CollectorLogEntry[] = []): ActivityLogStoreLike {
  return {
    load: async () => entries as Awaited<ReturnType<ActivityLogStoreLike['load']>>,
  };
}

// Minimal stub usage store
function makeUsageStore(totals: UsageTotals | null = null): UsageStoreLike {
  return {
    weeklyTotal: async () =>
      totals || {
        inputTokens: 0,
...
function makeAgentStore(config: AgentBudgetConfig = {}): AgentStoreLike {
  return {
    load: async () => ({
      weeklyTokenBudget: 0,
...
describe('collectWeeklyReviewData', () => {
  ...
  it('throws when weeklyPlanStore is not provided', async () => {
    await assert.rejects(
      () =>
        collectWeeklyReviewData(
          { activityLogStore: makeLogStore() } as unknown as Parameters<typeof collectWeeklyReviewData>[0],
          'agent-x',
          '2026-W16',
        ),
  ...
  it('throws when activityLogStore is not provided', async () => {
    await assert.rejects(
      () =>
        collectWeeklyReviewData(
          { weeklyPlanStore: makePlanStore() } as unknown as Parameters<typeof collectWeeklyReviewData>[0],
          'agent-x',
          '2026-W16',
        ),
  ...
  it('handles store errors gracefully (budget fields default to zero)', async () => {
    const failingUsageStore: UsageStoreLike = {
      weeklyTotal: async () => {
        throw new Error('disk read failure');
      },
    };
    const failingAgentStore: AgentStoreLike = {
      load: async () => {
