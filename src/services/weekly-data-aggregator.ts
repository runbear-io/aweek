...
interface WeeklyPlanStoreLike {
  load(agentId: string, week: string): Promise<WeeklyPlan>;
}

interface ActivityLogStoreLike {
  load(agentId: string, weekMonday: string): Promise<LogEntry[]>;
}
 ...
 */
export function weekDates(mondayDate: string): DayDate[] {
 ...
 */
export function bucketLogEntriesByDay(
  logEntries: LogEntry[],
  mondayDate: string,
): Map<string, LogEntry[]> {
...
export function bucketTasksByDay(
  tasks: PlanTask[],
  mondayDate: string,
): { byDay: Map<string, PlanTask[]>; unscheduled: PlanTask[] } {
  const days = weekDates(mondayDate);
  const dateToDay = new Map(days.map((d) => [d.date, d.day]));
 ...
 */
export function countByStatus(tasks: PlanTask[]): Record<string, number> {
 ...
 * @returns {object} Counts keyed by status
 */
export function countLogByStatus(entries: LogEntry[]): Record<string, number> {
  const counts: Record<string, number> = {};
...
export async function aggregateWeeklyData(
  { weeklyPlanStore, activityLogStore }: { weeklyPlanStore: WeeklyPlanStoreLike; activityLogStore: ActivityLogStoreLike },
  agentId: string,
  week: string,
  opts: AggregateOpts = {},
): Promise<WeeklyData> {
