...
import {
  createGoal as createGoalRaw,
  createObjective as createObjectiveRaw,
  createMonthlyPlan as createMonthlyPlanRaw,
} from '../models/agent.js';
import { validateWeeklyPlan } from '../schemas/validator.js';
import { isReviewObjectiveId } from '../schemas/weekly-plan.schema.js';

// Typed wrappers around the JSDoc-only `.js` factories — they actually
// return well-shaped objects, but their `@returns {object}` annotation
// strips that shape for TS callers.
interface TestGoal { id: string; description: string; horizon: string; status: string; createdAt: string; [key: string]: unknown }
interface TestObjective { id: string; description: string; goalId: string; status: string; [key: string]: unknown }
interface TestMonthlyPlan { month: string; objectives: TestObjective[]; status: string; createdAt: string; updatedAt: string; [key: string]: unknown }
const createGoal = (description: string, horizon?: '1mo' | '3mo' | '1yr'): TestGoal =>
  createGoalRaw(description, horizon) as unknown as TestGoal;
const createObjective = (description: string, goalId: string): TestObjective =>
  createObjectiveRaw(description, goalId) as unknown as TestObjective;
const createMonthlyPlan = (month: string, objectives: TestObjective[]): TestMonthlyPlan =>
  createMonthlyPlanRaw(month, objectives) as unknown as TestMonthlyPlan;
...
function makeEntry(status: string, title: string, timestamp?: string): TestLogEntry {
  return {
...
function makeReviewMarkdown({
  ...
  blockersBody = '',
  metricsBody = '',
}: { completedBody?: string; blockersBody?: string; metricsBody?: string } = {}): string {
  const lines = [
...
`;
  ...
  it('advisor brief includes retrospective bridge sentence when retrospectiveContext is supplied', () => {
    ...
    const workTask = plan.tasks.find((t: { objectiveId?: string }) => !isReviewObjectiveId(t.objectiveId));
