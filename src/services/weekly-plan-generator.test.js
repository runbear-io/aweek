...
`;
  ...
  it('priority-waterfall mode pre-sorts work tasks by priority (critical first, review tasks after)', () => {
    const goal = createGoal('G', '1mo');
    const objA = createObjective('Low priority work', goal.id);      // planned → medium
    const objB = createObjective('Critical path work', goal.id);
    objB.status = 'in-progress'; // → high
    const objC = createObjective('Ship MVP', goal.id);
    const mp = createMonthlyPlan('2026-04', [objA, objB, objC]);
  ...
  it('theme-days mode does NOT reorder tasks by priority', () => {
    ...
    const objA = createObjective('Medium task', goal.id);  // planned → medium
    const objB = createObjective('High task', goal.id);
    objB.status = 'in-progress'; // → high
    const mp = createMonthlyPlan('2026-04', [objA, objB]);
...
`;
  ...
  it('uses objective description directly when planMarkdown is absent (backward compat)', () => {
    const goal = createGoal('Ship a REST API', '1mo');
    const obj = createObjective('Implement endpoints', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('uses objective description directly when planMarkdown is null (backward compat)', () => {
    const goal = createGoal('Ship a REST API', '1mo');
    const obj = createObjective('Write documentation', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('generates an advisor brief (longer than raw description) when planMarkdown is supplied', () => {
    const goal = createGoal('Ship a production REST API', '1mo');
    const obj = createObjective('Implement endpoints', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('advisor brief still contains the objective description text', () => {
    ...
    const obj = createObjective('Build rate limiter', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('advisor brief references prior day outcomes when priorDayOutcomes is supplied', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj = createObjective('Add rate limiting', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('taskOverrides are NOT replaced by advisor brief composer (user intent preserved)', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj = createObjective('Build login', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('advisor brief task retains correct objectiveId traceability', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj = createObjective('Design schema', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('in-progress objective gets high priority in advisor brief task', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj = createObjective('Finish auth module', goal.id);
    obj.status = 'in-progress';
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('multiple objectives with planMarkdown all get advisor briefs', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj1 = createObjective('Build auth', goal.id);
    const obj2 = createObjective('Write docs', goal.id);
    obj2.status = 'in-progress';
    const mp = createMonthlyPlan('2026-04', [obj1, obj2]);
  ...
  it('generated plan with advisor briefs passes schema validation', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj = createObjective('Write API docs', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
  ...
  it('advisor brief composition does not affect review tasks (they keep their fixed descriptions)', () => {
    const goal = createGoal('Ship API', '1mo');
    const obj = createObjective('Build feature', goal.id);
    const mp = createMonthlyPlan('2026-04', [obj]);
