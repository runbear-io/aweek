...
interface AgentWeeklyPlanShape {
  week: string;
  month?: string | null;
  tasks: AgentTaskInPlan[];
  approved?: boolean;
  approvedAt?: string;
  updatedAt?: string;
}

interface AgentObjectiveShape {
  id: string;
  description?: string;
  goalId?: string;
}

interface AgentMonthlyPlanShape {
  month: string;
  objectives: AgentObjectiveShape[];
}

interface AgentGoalShape {
  id: string;
  description?: string;
}
...
interface AgentConfigShape {
  id: string;
  subagentRef?: string;
  goals: AgentGoalShape[];
  monthlyPlans?: AgentMonthlyPlanShape[];
  updatedAt?: string;
}
...
export interface ProcessApprovalResult {
  success: boolean;
  plan?: AgentWeeklyPlanShape & { _rejected?: boolean; _rejectionReason?: string | null };
  isFirstApproval?: boolean;
  editResults?: AppliedEdit[];
  errors?: string[];
}
...
export const APPROVAL_DECISIONS: readonly string[] = ['approve', 'reject', 'edit'];
 ...
 */
export function findPendingPlan(
  input: AgentWeeklyPlanShape[] | { weeklyPlans?: AgentWeeklyPlanShape[] } | null | undefined,
): { plan: AgentWeeklyPlanShape; week: string } | null {
  const plans: AgentWeeklyPlanShape[] | null = Array.isArray(input)
    ? input
    : input && Array.isArray((input as { weeklyPlans?: AgentWeeklyPlanShape[] }).weeklyPlans)
      ? (input as { weeklyPlans?: AgentWeeklyPlanShape[] }).weeklyPlans!
      : null;
  ...
  if (objectiveIds.length > 0) {
    ...
    for (const objId of objectiveIds) {
      ...
      for (const mp of agentConfig.monthlyPlans || []) {
        const obj = mp.objectives.find((o: AgentObjectiveShape) => o.id === objId);
        if (obj) {
          objDesc = `${objId}: ${obj.description}`;
          // Find the goal
          const goal = agentConfig.goals.find((g: AgentGoalShape) => g.id === obj.goalId);
          if (goal) {
...
export function validateEdits(
  ...
  for (const [i, edit] of (edits as PlanEditOp[]).entries()) {
    if (!edit || typeof edit !== 'object') {
      errors.push(`edit[${i}]: must be an object`);
      continue;
    }

    if (!edit.action || !validActions.includes(edit.action)) {
    ...
    if (edit.action === 'add') {
      ...
      if (!edit.objectiveId || typeof edit.objectiveId !== 'string') {
        ...
        let found = false;
        for (const mp of agentConfig.monthlyPlans || []) {
          if (mp.objectives.find((o: AgentObjectiveShape) => o.id === edit.objectiveId)) {
            found = true;
            break;
          }
        }
    ...
    if (edit.action === 'remove') {
      ...
      } else if (!plan.tasks.find((t: AgentTaskInPlan) => t.id === edit.taskId)) {
    ...
    if (edit.action === 'update') {
      ...
      if (edit.estimatedMinutes !== undefined) {
        const em = edit.estimatedMinutes;
        if (!Number.isInteger(em) || em < 1 || em > 480) {
...
export function applyEdits(
  plan: AgentWeeklyPlanShape,
  edits: PlanEditOp[],
): { applied: AppliedEdit[]; plan: AgentWeeklyPlanShape } {
  ...
  for (const edit of edits) {
    if (edit.action === 'add') {
      const task = createTask(
        ...
        edit.objectiveId,
        {
          priority: (edit.priority as 'critical' | 'high' | 'medium' | 'low') || 'medium',
          ...(edit.estimatedMinutes !== undefined ? { estimatedMinutes: edit.estimatedMinutes } : {}),
        },
      ) as AgentTaskInPlan;
    ...
    if (edit.action === 'remove') {
      const idx = plan.tasks.findIndex((t: AgentTaskInPlan) => t.id === edit.taskId);
      if (idx !== -1) {
    ...
    if (edit.action === 'update') {
      const task = plan.tasks.find((t: AgentTaskInPlan) => t.id === edit.taskId);
      if (task) {
...
export async function processApproval({
  ...
  rejectionReason,
  dataDir,
}: ProcessApprovalParams): Promise<ProcessApprovalResult> {
  // Validate decision
  ...
  let plans: AgentWeeklyPlanShape[];
  try {
    plans = (await weeklyPlanStore.loadAll(agentId)) as AgentWeeklyPlanShape[];
  } catch {
    plans = [];
  }
  ...
  if (decision === 'edit') {
    ...
    // Apply edits
    const { applied } = applyEdits(plan, edits as PlanEditOp[]);

    // Validate the modified plan against schema
    const schemaResult = validateWeeklyPlan(plan) as { valid: boolean; errors: { instancePath?: string; message?: string }[] };
 ...
 */
export function approve(params: Omit<ProcessApprovalParams, 'decision'> = {} as Omit<ProcessApprovalParams, 'decision'>): Promise<ProcessApprovalResult> {
  return processApproval({ ...params, decision: 'approve' } as ProcessApprovalParams);
}
 ...
 * @returns {Promise<ReturnType<typeof processApproval>>}
 */
export function reject(params: Omit<ProcessApprovalParams, 'decision'> = {} as Omit<ProcessApprovalParams, 'decision'>): Promise<ProcessApprovalResult> {
  return processApproval({ ...params, decision: 'reject' } as ProcessApprovalParams);
}
 ...
 * @returns {Promise<ReturnType<typeof processApproval>>}
 */
export function edit(params: Omit<ProcessApprovalParams, 'decision'> = {} as Omit<ProcessApprovalParams, 'decision'>): Promise<ProcessApprovalResult> {
  return processApproval({ ...params, decision: 'edit' } as ProcessApprovalParams);
}
 ...
 * @returns {string}
 */
export function formatApprovalResult(result: ProcessApprovalResult, decision: string): string {
  if (!result.success) {
 ...
 */
export interface LoadPlanForReviewParams {
  agentId: string;
  dataDir?: string;
}

export interface LoadPlanForReviewResult {
  success: boolean;
...
export async function loadPlanForReview(
  { agentId, dataDir }: LoadPlanForReviewParams,
): Promise<LoadPlanForReviewResult> {
  const store = createAgentStore(dataDir);
  let config: AgentConfigShape;
  try {
    config = (await store.load(agentId)) as unknown as AgentConfigShape;
