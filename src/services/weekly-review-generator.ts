...
interface WeeklyPlanStoreLike {
  load(agentId: string, week: string): Promise<WeeklyPlanInput>;
}

interface ActivityLogStoreLike {
  load(agentId: string, weekMonday: string): Promise<LogEntryInput[]>;
}

interface CollectedWorkTask {
  id?: string;
  title?: string;
  status?: string;
  priority?: string;
...
interface CollectedDataLike {
  plan?: { workTasks?: CollectedWorkTask[] } | null;
  activityLog?: { totalDurationMs?: number; byStatus?: Record<string, unknown> } | null;
 ...
 */
export function collectCompletedTasksFromPlan(
  weeklyPlan: WeeklyPlanInput | null | undefined,
): CollectedPlanTask[] {
 ...
 */
export function collectCompletedFromActivityLog(
  logEntries: LogEntryInput[] | null | undefined,
): CollectedLogEntry[] {
...
export function mergeCompletedTasks(
  planTasks: CollectedPlanTask[],
  logEntries: CollectedLogEntry[],
): MergedTask[] {
  const merged: MergedTask[] = [];
  const usedLogIds = new Set<string | undefined>();
 ...
 */
export function formatCompletedTaskItem(
  task: MergedTask,
  opts: FormatItemOpts = {},
): string {
 ...
 */
export function formatCompletedTasksSection(
  completedTasks: MergedTask[],
  opts: FormatSectionOpts = {},
): string {
  const {
  ...
  if (groupByObjective) {
    ...
    const groups = new Map<string, MergedTask[]>();
    const ungrouped: MergedTask[] = [];
...
interface CompletedTasksReviewDeps {
...
export async function generateCompletedTasksReview(
  { weeklyPlanStore, activityLogStore }: CompletedTasksReviewDeps,
  agentId: string,
  week: string,
  weekMonday: string,
  opts: FormatSectionOpts = {},
): Promise<{ completedTasks: MergedTask[]; markdown: string }> {
  // Load data from both sources
  let planTasks: CollectedPlanTask[] = [];
  let logCompletions: CollectedLogEntry[] = [];
 ...
 * @returns {string}
 */
function formatCostLocal(usd: number | null | undefined): string {
...
export function formatTaskStatusSection(
  workTasks: CollectedWorkTask[] | null | undefined,
): string {
  const lines: string[] = [];
 ...
 * @returns {string} Markdown for the section
 */
export function formatCarryOverSection(
  workTasks: CollectedWorkTask[] | null | undefined,
): string {
  const lines: string[] = [];
 ...
 * @returns {string} Markdown for the section
 */
export function formatWhatWorkedSection(
  collectedData: CollectedDataLike | null | undefined,
): string {
  ...
  const byObj = new Map<string, CollectedWorkTask[]>();
 ...
 * @returns {string} Markdown for the section
 */
export function formatBudgetSummarySection(
  ...
  if (!budget || (budget.totalTokens ?? 0) === 0) {
  ...
  if ((budget.weeklyTokenLimit ?? 0) > 0) {
    lines.push(`| Weekly token limit | ${formatN(budget.weeklyTokenLimit)} |`);
    if (budget.remainingTokens !== null && budget.remainingTokens !== undefined) {
      lines.push(`| Remaining tokens | ${formatN(budget.remainingTokens)} |`);
    }
    if (budget.utilizationPct !== null && budget.utilizationPct !== undefined) {
      const indicator =
 ...
 */
export function generateWeeklyReviewContent(
  collectedData: CollectedDataLike | null | undefined,
