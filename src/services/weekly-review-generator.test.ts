...
function makePlan(tasks: TestTask[] = [], overrides: Partial<TestPlan> = {}): TestPlan {
  return {
    week: '2026-W16',
    month: '2026-04',
    tasks,
...
function makeTask(id: string, status: string = 'completed', overrides: Partial<TestTask> = {}): TestTask {
...
function makeLogEntry(id: string, status: string = 'completed', overrides: Partial<TestLogEntry> = {}): TestLogEntry {
  const text = overrides.title || overrides.description || `Log entry ${id}`;
  const base: TestLogEntry = {
    id,
...
describe('formatCompletedTasksSection', () => {
  ...
  it('includes summary count', () => {
    const tasks = [
      { taskId: null, description: 'A', objectiveId: 'obj-aaa11111', priority: 'medium', completedAt: '2026-04-14T10:00:00.000Z', estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
      { taskId: null, description: 'B', objectiveId: 'obj-aaa11111', priority: 'medium', completedAt: '2026-04-15T10:00:00.000Z', estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
    ];
  ...
  it('includes total duration in summary', () => {
    const tasks = [
      { taskId: null, description: 'A', objectiveId: null, priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: 120000, source: 'merged' },
      { taskId: null, description: 'B', objectiveId: null, priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: 180000, source: 'merged' },
    ];
    const md = formatCompletedTasksSection(tasks);
  ...
  it('singular "task" for single completion', () => {
    const tasks = [
      { taskId: null, description: 'Only one', objectiveId: null, priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
    ];
    const md = formatCompletedTasksSection(tasks);
  ...
  it('groups tasks by objective', () => {
    const tasks = [
      { taskId: null, description: 'A', objectiveId: 'obj-aaa11111', priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
      { taskId: null, description: 'B', objectiveId: 'obj-bbb22222', priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
      { taskId: null, description: 'C', objectiveId: 'obj-aaa11111', priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
    ];
    const md = formatCompletedTasksSection(tasks);
  ...
  it('puts tasks without objective under "Other Completed Work"', () => {
    const tasks = [
      { taskId: null, description: 'Linked', objectiveId: 'obj-aaa11111', priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
      { taskId: null, description: 'Orphan', objectiveId: null, priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
    ];
    const md = formatCompletedTasksSection(tasks);
  ...
  it('omits summary when includeSummary=false', () => {
    const tasks = [
      { taskId: null, description: 'A', objectiveId: null, priority: 'medium', completedAt: null, estimatedMinutes: null, durationMs: null, source: 'weekly-plan' },
    ];
...
type ReviewDeps = Parameters<typeof generateCompletedTasksReview>[0];
...
describe('generateCompletedTasksReview', () => {
  ...
  function deps(): ReviewDeps {
    return {
      weeklyPlanStore: weeklyPlanStore as unknown as ReviewDeps['weeklyPlanStore'],
      activityLogStore: activityLogStore as unknown as ReviewDeps['activityLogStore'],
    };
  }
...
type CollectedShape = Parameters<typeof formatWhatWorkedSection>[0];

describe('formatWhatWorkedSection', () => {
  function makeCollected(workTasks: TestTask[] = [], totalDurationMs: number = 0, byStatus: Record<string, unknown> = {}): CollectedShape {
    return {
      plan: { workTasks },
      activityLog: { totalDurationMs, byStatus },
    } as unknown as CollectedShape;
  }
...
describe('formatBudgetSummarySection', () => {
  function makeBudget(overrides: Partial<BudgetShape> = {}): BudgetShape {
    return {
      weeklyTokenLimit: 0,
      inputTokens: 30000,
      outputTokens: 20000,
...
type FullCollectedArg = Parameters<typeof generateWeeklyReviewContent>[0];

describe('generateWeeklyReviewContent', () => {
  function makeCollectedData(overrides: Partial<FullCollected> = {}): FullCollectedArg {
    const base = makeCollectedDataBase(overrides);
    return base as unknown as FullCollectedArg;
  }

  function makeCollectedDataBase(overrides: Partial<FullCollected> = {}): FullCollected {
    return {
      agentId: 'agent-test-1234abcd',
      week: '2026-W16',
