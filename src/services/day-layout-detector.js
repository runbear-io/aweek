 ...
 * @type {Array<{ name: string, pattern: RegExp }>}
 */
const PRIORITY_WATERFALL_SIGNALS = [
  {
 ...
 */
export function scoreThemeDays(text) {
  let score = 0;
 ...
 */
export function scorePriorityWaterfall(text) {
  let score = 0;
 ...
 */
export function detectDayLayout(planMarkdown) {
 ...
 * }}
 */
export function detectDayLayoutWithConfidence(planMarkdown) {
  if (typeof planMarkdown !== 'string' || planMarkdown.trim().length === 0) {
    return {
      mode: 'mixed',
      confident: false,
 ...
 * @type {Set<string>}
 */
export const LAYOUT_MODES = new Set(['theme-days', 'priority-waterfall', 'mixed']);
