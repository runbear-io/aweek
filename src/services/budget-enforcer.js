 ...
 * @returns {string}
 */
export function alertsDir(baseDir, agentId) {
 ...
 * @returns {string}
 */
export function alertFilePath(baseDir, agentId, weekMonday) {
 ...
 */
async function alertExists(filePath) {
  try {
 ...
 */
async function writeAlert(filePath, details) {
 ...
 */
export async function enforceBudget(agentId, deps, weekMonday, timestamp) {
 ...
 * @returns {Promise<EnforcementResult[]>}
 */
export async function enforceAllBudgets(deps, weekMonday) {
  if (!deps?.agentStore) {
    throw new Error('agentStore dependency is required');
  }
 ...
 * @returns {Promise<boolean>}
 */
export async function isAgentPaused(agentId, deps) {
  if (!deps?.agentStore) {
    throw new Error('agentStore dependency is required');
  }
  const config = await deps.agentStore.load(agentId);
 ...
 */
export async function resumeAgent(agentId, deps) {
  if (!deps?.agentStore) {
    throw new Error('agentStore dependency is required');
  }
  return deps.agentStore.update(agentId, (cfg) => {
 ...
 */
export async function topUpResume(agentId, deps, options = {}) {
  if (!agentId || typeof agentId !== 'string') {
    throw new Error('agentId is required and must be a string');
  }
  if (!deps?.agentStore) {
  ...
  await deps.agentStore.update(agentId, (cfg) => {
    // Reset usage to zero
 ...
 */
export async function loadAlert(baseDir, agentId, weekMonday) {
  const monday = weekMonday || getMondayDate();
...
export function createBudgetEnforcer(config = {}) {
  ...
  const deps = { agentStore, usageStore, baseDir: config.baseDir };

  return {
    enforce: (agentId, weekMonday, timestamp) => enforceBudget(agentId, deps, weekMonday, timestamp),
    enforceAll: (weekMonday) => enforceAllBudgets(deps, weekMonday),
    isPaused: (agentId) => isAgentPaused(agentId, deps),
    resume: (agentId) => resumeAgent(agentId, deps),
    topUp: (agentId, options) => topUpResume(agentId, deps, options),
    loadAlert: (agentId, weekMonday) => loadAlert(deps.baseDir || agentStore.baseDir, agentId, weekMonday),
  };
}
