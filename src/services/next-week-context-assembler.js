 ...
 */
export function extractRetrospectiveSummary(reviewMarkdown) {
 ...
 */
export function summariseActivityLog(entries) {
 ...
 * }>}
 */
export async function assembleNextWeekPlannerContext(deps, agentId, week, opts = {}) {
  const {
    agentsDir = null,
    baseDir = null,
  ...
  const retrospectiveMarkdown = reviewResult?.markdown ?? null;
  const retrospectiveContext = extractRetrospectiveSummary(retrospectiveMarkdown);
  const activityLogSummary = summariseActivityLog(activityLogEntries ?? []);
