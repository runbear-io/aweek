 ...
 */
interface GoalShape { id: string; description: string; horizon: string; status?: string; }
interface ObjectiveShape { id: string; description: string; goalId: string; }
interface MonthlyPlanShape { month: string; objectives: ObjectiveShape[]; }
interface TaskShape {
  id: string;
  title: string;
  prompt: string;
...
interface AgentConfigShape {
  ...
  subagentRef?: string;
  goals: GoalShape[];
  monthlyPlans: MonthlyPlanShape[];
  weeklyTokenBudget?: number;
  budget?: { weeklyTokenLimit: number; currentUsage: number; periodStart: string; paused?: boolean; sessions?: unknown[] };
  inbox?: unknown[];
  identity?: unknown;
  name?: string;
  role?: string;
...
interface BuildTestAgentResult {
  config: AgentConfigShape;
  weeklyPlans: WeeklyPlanShape[];
  goal: GoalShape;
  obj: ObjectiveShape;
  monthlyPlan: MonthlyPlanShape;
  weeklyPlan: WeeklyPlanShape;
  task1: TaskShape;
  task2: TaskShape;
}

function buildTestAgent({ subagentRef = TEST_SLUG }: { subagentRef?: string } = {}): BuildTestAgentResult {
  const config = createAgentConfig({ subagentRef }) as AgentConfigShape;

  const goal = createGoal('Build a great product', '3mo') as GoalShape;
  config.goals.push(goal);

  const obj = createObjective('Implement core features', goal.id) as ObjectiveShape;
  const monthlyPlan = createMonthlyPlan('2026-04', [obj]) as MonthlyPlanShape;
  config.monthlyPlans.push(monthlyPlan);

  const task1 = createTask({ title: 'Design database schema', prompt: 'Design database schema' }, obj.id, { priority: 'high' as const, estimatedMinutes: 60 }) as TaskShape;
  const task2 = createTask({ title: 'Write API endpoints', prompt: 'Write API endpoints' }, obj.id, { priority: 'medium' as const, estimatedMinutes: 120 }) as TaskShape;
  const weeklyPlan = createWeeklyPlan('2026-W16', '2026-04', [task1, task2]) as WeeklyPlanShape;
  ...
  const weeklyPlans: WeeklyPlanShape[] = [weeklyPlan];
 ...
 */
async function saveTestAgent(config: AgentConfigShape, weeklyPlans: WeeklyPlanShape[] = []) {
...
describe('findPendingPlan', () => {
  ...
  it('skips approved plans and finds the pending one', () => {
    ...
    const approvedPlan = createWeeklyPlan('2026-W15', '2026-04', []) as WeeklyPlanShape;
...
describe('formatPlanForReview', () => {
  ...
  it('handles plan with no tasks', () => {
    ...
    const emptyPlan = createWeeklyPlan('2026-W17', '2026-04', []) as WeeklyPlanShape;
    const text = formatPlanForReview(config, emptyPlan as Parameters<typeof formatPlanForReview>[1]);
...
describe('applyEdits', () => {
  ...
  it('updates a task title', () => {
    ...
    const task = plan.tasks.find((t: TaskShape) => t.id === task1.id);
  ...
  it('updates task priority', () => {
    ...
    const task = weeklyPlan.tasks.find((t: TaskShape) => t.id === task1.id);
...
describe('processApproval — approve', () => {
  ...
  it('detects subsequent approval (not first)', async () => {
    ...
    const oldPlan = createWeeklyPlan('2026-W15', '2026-04', []) as WeeklyPlanShape;
  ...
  it('persists approval to file', async () => {
    ...
    // Reload and verify
    const weeklyPlanStore = new WeeklyPlanStore(tmpDir);
    const plans = await weeklyPlanStore.loadAll(config.id) as WeeklyPlanShape[];
...
describe('processApproval — reject', () => {
  it('removes the pending plan', async () => {
    ...
    const weeklyPlanStore = new WeeklyPlanStore(tmpDir);
    const remainingPlans = await weeklyPlanStore.loadAll(config.id) as WeeklyPlanShape[];
...
describe('processApproval — edit', () => {
  ...
  it('persists edits to file', async () => {
    ...
    const weeklyPlanStore = new WeeklyPlanStore(tmpDir);
    const editedPlan = await weeklyPlanStore.load(config.id, '2026-W16') as WeeklyPlanShape;
...
describe('formatApprovalResult', () => {
  it('formats approve result', () => {
    const text = formatApprovalResult({
      success: true,
      plan: { week: '2026-W16', tasks: [{ id: 't1' }, { id: 't2' }], approvedAt: '2026-04-16T10:00:00Z' } as unknown as WeeklyPlanShape,
  ...
  it('formats first approval with project-heartbeat notice', () => {
    const text = formatApprovalResult({
      success: true,
      plan: { week: '2026-W16', tasks: [], approvedAt: '2026-04-16T10:00:00Z' } as unknown as WeeklyPlanShape,
  ...
  it('formats reject result', () => {
    const text = formatApprovalResult({
      success: true,
      plan: { _rejected: true, _rejectionReason: 'Too vague' } as unknown as WeeklyPlanShape,
    }, 'reject');
  ...
  it('formats edit result', () => {
    const text = formatApprovalResult({
      success: true,
      plan: { approved: false } as unknown as WeeklyPlanShape,
      editResults: [
  ...
  it('formats edit with auto-approve and first approval', () => {
    const text = formatApprovalResult({
      success: true,
      plan: { approved: true } as unknown as WeeklyPlanShape,
...
describe('Subagent-wrapper invariant — aweek JSON never reintroduces identity', () => {
  it('approve mutates scheduling fields only; id/subagentRef/createdAt unchanged', async () => {
    const ctx = await scaffoldProjectWithSubagentMd();
    const before = await ctx.store.load(ctx.config.id) as AgentConfigShape;
    ...
    const after = await ctx.store.load(ctx.config.id) as AgentConfigShape;
    ...
    const afterPlans = await ctx.weeklyPlanStore.loadAll(ctx.config.id) as WeeklyPlanShape[];
  ...
  it('reject removes pending plan but preserves id/subagentRef/createdAt', async () => {
    ...
    const afterPlansR = await ctx.weeklyPlanStore.loadAll(ctx.config.id) as WeeklyPlanShape[];
  ...
  it('edit mutates only plan.tasks; id/subagentRef/createdAt unchanged', async () => {
    const ctx = await scaffoldProjectWithSubagentMd();
    const before = await ctx.store.load(ctx.config.id) as AgentConfigShape;
    const beforePlans = await ctx.weeklyPlanStore.loadAll(ctx.config.id) as WeeklyPlanShape[];
    ...
    const after = await ctx.store.load(ctx.config.id) as AgentConfigShape;
    const afterPlansE = await ctx.weeklyPlanStore.loadAll(ctx.config.id) as WeeklyPlanShape[];
    assert.equal(after.id, before.id);
    assert.equal(after.subagentRef, before.subagentRef);
    assert.equal(after.createdAt, before.createdAt);
