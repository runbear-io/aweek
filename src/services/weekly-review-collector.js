 ...
 */
async function safeLoadPlan(weeklyPlanStore, agentId, week) {
  try {
 ...
 */
async function safeLoadLog(activityLogStore, agentId, weekMonday) {
  try {
 ...
 */
async function safeLoadUsageTotals(usageStore, agentId, weekMonday) {
  const zero = {
    inputTokens: 0,
    outputTokens: 0,
    totalTokens: 0,
 ...
 */
async function safeLoadAgentBudgetConfig(agentStore, agentId) {
 ...
 */
export function splitTasksByType(tasks) {
 ...
 */
export function groupLogEntriesByStatus(entries) {
  const groups = {};
 ...
 */
export function computeBudgetUtilization(totalTokens, weeklyTokenLimit) {
  if (!weeklyTokenLimit) {
...
/**
 * @typedef {object} CollectedWeekData
 * @property {string} agentId
 * @property {string} week
 * @property {string} weekMonday
 * @property {string} collectedAt
 * @property {object} plan
 * @property {boolean} plan.exists
 * @property {boolean} plan.approved
 * @property {string|null} plan.createdAt
 * @property {object[]} plan.allTasks
 * @property {object[]} plan.workTasks
 * @property {object[]} plan.reviewTasks
 * @property {object} activityLog
 * @property {object[]} activityLog.entries
 * @property {object} activityLog.byStatus
 * @property {number} activityLog.totalDurationMs
 * @property {object} budget
 * @property {number} budget.weeklyTokenLimit
 * @property {number} budget.inputTokens
 * @property {number} budget.outputTokens
 * @property {number} budget.totalTokens
 * @property {number} budget.costUsd
 * @property {number} budget.sessionCount
 * @property {number|null} budget.remainingTokens
 * @property {number|null} budget.utilizationPct
 * @property {boolean} budget.paused
 */
 ...
 */
export async function collectWeeklyReviewData(deps, agentId, week, opts = {}) {
