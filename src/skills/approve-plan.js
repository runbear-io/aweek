/**
 * Approve-plan skill logic.
 * Loads an agent's pending weekly plan, presents it for review,
 * and handles approval, rejection, or edits before persisting.
 *
 * This is the human-in-the-loop gate — first approval triggers heartbeat activation.
 *
 * Approval flow:
 *  1. Load agent and find the pending (unapproved) weekly plan
 *  2. Format the plan for human review
 *  3. Accept one of: approve, reject, or edit (modify tasks)
 *  4. Persist the result and return status
 *
 * Used by the /aweek:approve-plan Claude Code skill.
 */
